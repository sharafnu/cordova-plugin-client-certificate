...
- (void)registerAuthenticationCertificate:(CDVInvokedUrlCommand*)command
{
    ...
    NSString* finalpath = [certPathName stringByAppendingString:[@"/" stringByAppendingString:path]];
    NSLog(@"@", finalpath);
    ...
    SecTrustRef myTrust;
    OSStatus status = extractIdentityAndTrust(finalpath, password, &myIdentity, &myTrust);
    if(status != noErr) {
